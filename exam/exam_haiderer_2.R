...
for (i in colnames(a_means)){
  print(i)
  x_balance <- ((a_means[,i]/a_cd[i,"molweight"]) * a_cd[i,"carbons"])
  print(x_balance)
}
...
getCarbonBalance <- function(x, y, cumulate="co2"){
  x_means <- getBatchMeans(x, na.rm=T)
  # cs <- cumulate
 ...
 # for (el in 1:9){
  ...
  print(x_balance)
  test <- cbind( x_balance)
  rownames(test) <- x_means[,"hour"]
  test <- test[,-1]
  return(test)
  }
  #return(test)
...
mytest <- getCarbonBalance(a_wo_outlier,a_cd,cumulate="co2")
